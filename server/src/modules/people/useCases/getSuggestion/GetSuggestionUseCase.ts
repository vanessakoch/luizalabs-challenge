...
class GetSuggestionUseCase {
	...
	async execute(name: string): Promise<string[]> {
		...
		peopleRequest.friends.forEach(friend => myFriendsId.push(friend.id as string))
		
		// Buscar o objeto completo dos amigos da própria pessoa
		const myFriendsList: People[] = await this.peopleRepository.getMyFriendsById(myFriendsId)
	...
	getDiffFriends(myFriends: People[], suggestions: People[]) {
